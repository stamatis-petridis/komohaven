...
const CTA_DEFAULT_MESSAGE =
  "Select your stay dates to book or message us.";
...
async function enhanceBlock(root, slug) {
  ...
  try {
    ...
    updateAvailabilityBadge(root, updatedLabel);
    ...
    updateAvailabilityBadge(root, null);
...
// Update the heading badge to reflect the last refresh timestamp.
function updateAvailabilityBadge(root, updatedLabel) {
  if (!root) return;
  const badge = root.querySelector("[data-availability-updated]");
  if (!badge) return;
...
function resolveNightlyRateCents(slug) {
  ...
  const api =
    ...
    window.KOMO_RATES &&
    typeof window.KOMO_RATES.getNightlyRateCents === "function"
      ? window.KOMO_RATES.getNightlyRateCents
      : null;
  if (!api) return null;
  try {
    const rate = api(key);
...
function formatEuroFromCents(cents) {
  ...
  return EURO_FORMATTER.format(amount / 100);
}

function buildPricingDetails(state, startDate, endDate) {
  if (!state) return "";
  const nights = calculateNightCount(startDate, endDate);
  if (!nights) return "";
  const slug = state.rateSlug || "";
  let rateCents =
    typeof state.rateCents === "number" ? state.rateCents : null;
  if (rateCents === null) {
    rateCents = resolveNightlyRateCents(slug);
    if (typeof rateCents === "number") {
...
function setCTASelectedState(state, propertyLabel, startDate, endDate) {
  ...
  if (state.summaryEl) {
    const baseText = `Book ${labelText} from ${startLabel} to ${endLabel}`;
    const pricingDetails = buildPricingDetails(state, startDate, endDate);
    state.summaryEl.textContent = pricingDetails
      ? `${baseText} ${pricingDetails}`
      : baseText;
  }
  if (state.bookBtn) {
    state.bookBtn.disabled = false;
  }
  if (state.sendBtn) {
...
function ensureAvailabilityCTA(root) {
  ...
  if (!bar) {
    ...
    bookBtn.setAttribute("data-availability-book", "");
  ...
  let actionsEl = bar.querySelector(".availability-cta-actions");
  if (!actionsEl) {
    actionsEl = document.createElement("div");
    actionsEl.className = "availability-cta-actions";
    bar.appendChild(actionsEl);
  }
  let bookBtnEl = bar.querySelector("[data-availability-book]");
  if (!bookBtnEl) {
    bookBtnEl = document.createElement("button");
    bookBtnEl.type = "button";
    bookBtnEl.className = "btn primary";
    bookBtnEl.setAttribute("data-availability-book", "");
    bookBtnEl.textContent = "Book Now";
    bookBtnEl.disabled = true;
    actionsEl.insertBefore(bookBtnEl, actionsEl.firstChild);
  } else {
    bookBtnEl.classList.add("btn", "primary");
  }
  let sendBtnEl = bar.querySelector("[data-availability-send]");
  if (!sendBtnEl) {
    sendBtnEl = document.createElement("button");
    sendBtnEl.type = "button";
    sendBtnEl.className = "btn";
    sendBtnEl.setAttribute("data-availability-send", "");
    sendBtnEl.textContent = "Send via WhatsApp";
    sendBtnEl.disabled = true;
    actionsEl.insertBefore(sendBtnEl, actionsEl.querySelector("[data-availability-cancel]"));
  } else {
    sendBtnEl.classList.add("btn");
  }
  let cancelBtnEl = bar.querySelector("[data-availability-cancel]");
  if (!cancelBtnEl) {
    cancelBtnEl = document.createElement("button");
    cancelBtnEl.type = "button";
    cancelBtnEl.className = "btn outline";
    cancelBtnEl.setAttribute("data-availability-cancel", "");
    cancelBtnEl.textContent = "Clear";
    cancelBtnEl.disabled = true;
    actionsEl.appendChild(cancelBtnEl);
  } else {
    cancelBtnEl.classList.add("btn", "outline");
  }
  if (summaryEl) {
...
function setupRangeSelection(
  ...
  const rootSlug =
    root && typeof root.getAttribute === "function"
      ? root.getAttribute("data-availability-slug")
      : "";
  const rateSlug = (slug || rootSlug || "").toLowerCase();
  ...
  if (!state) {
    ...
    state.rateSlug = (rateSlug || state.rateSlug || "").toLowerCase();
    const latestRate = resolveNightlyRateCents(state.rateSlug);
    if (typeof latestRate === "number") {
      state.rateCents = latestRate;
    } else if (typeof state.rateCents !== "number") {
      state.rateCents = null;
    }
  ...
  if (state.bookBtn && !state.bookBtn.dataset.availabilityBound) {
    state.bookBtn.dataset.availabilityBound = "true";
    state.bookBtn.addEventListener("click", () => {
